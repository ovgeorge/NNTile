                    ...
                    for(Index j1 = 0; j1 < kernel_n; ++j1)
                    {
                        ...
                        for(Index i2 = 0; i2 < src_m; ++i2)
                        {
                            for(Index j2 = 0; j2 < kernel_m; ++j2)
                            {
                                ...
                                Y src_val =
                                    Y{src[i1 * src_m + i2 + ic * src_n * src_m +
                                          b * in_channels * src_n * src_m]};
                                Y kernel_val =
                                    Y{kernel[j1 * kernel_m + j2 +
...
template void
cpu<fp32_fast_tf32_t>(Index offset_n, Index offset_m, Index batch,
                      Index out_channels, Index in_channels, Index src_n,
                      Index src_m, const fp32_fast_tf32_t *src, Index kernel_n,
                      Index kernel_m, const fp32_fast_tf32_t *kernel,
                      Index dst_n, Index dst_m, fp32_fast_tf32_t *dst) noexcept;

template void cpu<bf16_t>(Index offset_n, Index offset_m, Index batch,
                          Index out_channels, Index in_channels, Index src_n,
                          Index src_m, const bf16_t *src, Index kernel_n,
                          Index kernel_m, const bf16_t *kernel, Index dst_n,
                          Index dst_m, bf16_t *dst) noexcept;
